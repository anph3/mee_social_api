...
class AuthView(ViewSet):
    ...
    def login(self, request):
        ...
        if not validate.is_valid():
            return rs.validate_error(validate.errors)
        ...
        if not user.exists():
            return rs.response_data(status=vs.STATUS['NO_DATA'], message=str(username) + vs.ERROR['not_exists'])
        
        # kiem tra password
        b_password = user.values(vs.P)[0][vs.P]
        if not self.check_passwork(password, b_password):
            return rs.response_data(status=vs.STATUS['FAIL_REQUEST'],message=vs.ERROR['wrong_password'])
    ...
    def register(self, request):
        ...
        if not data_save.is_valid():
            return rs.validate_error(data_save.errors)
        data_save.save()
        return rs.response_data(data_save.data)
    ...
    def refresh_token(self, request):
        ...
        if redis_data is None:
            return rs.response_data(status=vs.STATUS['INPUT_INVALID'],message=vs.ERROR['refresh_token'])
